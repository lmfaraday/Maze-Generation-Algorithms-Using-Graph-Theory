...
class SidewinderMaze:
    def __init__(self, width, height):
        self.maze_name = "Sidewinder Algorithm"
        self.width = width
        self.height = height
    ...
    def _generate_maze(self):
        for y in range(self.height):
            ...
            for x in range(self.width):
                ...
                carve_east = (x < self.width - 1) and (y == 0 or random.choice([True, False]))

                if carve_east:
                    neighbor = (x + 1, y)
                    self.maze.add_edge((x, y), neighbor)
                    self._update_grid((x, y), neighbor)
                else:
                    if run:
                        random_cell = random.choice(run)
                        if random_cell[1] > 0:
                            neighbor = (random_cell[0], random_cell[1] - 1)
                            self.maze.add_edge(random_cell, neighbor)
                            self._update_grid(random_cell, neighbor)
                    run = []
