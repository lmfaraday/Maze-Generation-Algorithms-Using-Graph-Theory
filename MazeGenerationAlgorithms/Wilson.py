...
class WilsonMaze:
    def __init__(self, width, height):
        self.maze_name = "Wilson's Algorithm"
        self.width = width
        self.height = height
    ...
    def _generate_maze(self):
        ...
        while len(visited_cells) < self.width * self.height:
            current_cell = random.choice([cell for cell in self.graph.nodes if cell not in visited_cells])
            path = [current_cell]

            while current_cell not in visited_cells:
                possible_neighbors = list(self.graph.neighbors(current_cell))
            ...
            for i in range(len(path) - 1):
                self.maze.add_edge(path[i], path[i + 1])
                self._update_grid(path[i], path[i + 1])
                self.frames.append(self.grid.copy())
                visited_cells.add(path[i])
