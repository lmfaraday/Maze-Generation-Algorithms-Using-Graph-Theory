...
class RecursiveBacktrackerMaze:
    def __init__(self, width, height):
        self.maze_name = "Recursive Backtracker Algorithm"
        self.width = width
        self.height = height
    ...
    def _generate_maze(self):
        ...
        self.maze.add_node(self.start)

        while stack:
            current = stack[-1]
            unvisited_neighbors = [n for n in self.graph.neighbors(current) if n not in visited]

            if unvisited_neighbors:
                next_cell = random.choice(unvisited_neighbors)
                self.maze.add_edge(current, next_cell)
                self._update_grid(current, next_cell)
